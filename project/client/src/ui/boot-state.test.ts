...
describe('bootReducer', () => {
  test('BOOT_START resets to loading/assets', () => {
    const prev: BootState = { status: 'error', phase: null, message: 'x' };
    const next = bootReducer(prev, { type: 'BOOT_START' });
    expect(next).toEqual({ status: 'loading', phase: 'assets', message: null });
  });
  ...
  test('BOOT_PHASE updates phase only when loading', () => {
    ...
    const ready: BootState = { status: 'ready', phase: null, message: null };
  ...
  test('BOOT_READY sets ready and clears message/phase', () => {
    const next = bootReducer(
      { status: 'loading', phase: 'webgl', message: null },
      { type: 'BOOT_READY' },
    );
    expect(next).toEqual({ status: 'ready', phase: null, message: null });
  });

  test('BOOT_ERROR sets error and clears phase', () => {
    const next = bootReducer(
      { status: 'loading', phase: 'webgl', message: null },
      { type: 'BOOT_ERROR', message: 'Nope' },
    );
    expect(next).toEqual({ status: 'error', phase: null, message: 'Nope' });
  });
});
